...
- `WP-GOV-SMOKE-001` remains `IN-PROGRESS`. Its baseline packaged-build evidence lives under `.product/build_target/tool_artifacts/manual_smoke/WP-GOV-SMOKE-001/20260309_202006/`, its follow-up current-code evidence lives under `.product/build_target/tool_artifacts/manual_smoke/WP-GOV-SMOKE-001/20260309_234318/`, and its current portable-build post-map-family evidence lives under `.product/build_target/tool_artifacts/manual_smoke/WP-GOV-SMOKE-001/20260311_194005/`. Together they show the first-use shell improved materially, stayed coherent after the map-family rollout, and now support confirmed 3D, bundle, export, reopen, and planning/solver flows on the packaged app; the remaining step is user-review judgment rather than missing basic flow proof.
...
- `WP-I1-010` is now `E2E-VERIFIED`; proof: `.product/build_target/tool_artifacts/wp_runs/WP-I1-010/20260311_124734/`. It closes the first live mobility family with OpenSky-backed Tauri fetches, truthful live/delayed/cached badges, a separate heuristic awareness overlay, projected map tracks, and recorder/bundle restore of the current air snapshot.
- `WP-I1-011` is now `E2E-VERIFIED`; proof: `.product/build_target/tool_artifacts/wp_runs/WP-I1-011/20260311_150847/`. It closes the satellite family with governed CelesTrak-backed orbital refresh in Tauri, explicit propagated/modeled labels, 2D/3D projected tracks and footprints, warm/cold runtime smoke proof, and deterministic recorder/bundle restore.
- `WP-GOV-MAPDATA-002` is now `E2E-VERIFIED`; proof: `.product/build_target/tool_artifacts/wp_runs/WP-GOV-MAPDATA-002/20260311_155444/`. It closes the maritime source-truth research seam by governing delayed/regional/cached maritime as the acceptable first implementation scope, allowing only backend-only user-key community live enhancement, and reserving any default global low-latency live claim for a separately governed licensed-provider path.
- `WP-I1-012` is now `E2E-VERIFIED`; proof: `.product/build_target/tool_artifacts/wp_runs/WP-I1-012/20260311_162912/`. It closes the constrained maritime family with cached benchmark vessel movement, separate port-awareness cues, truthful source-state wording, AOI-focused map projection, and recorder/bundle restore while keeping default global live and naval claims out of scope.
- `WP-I1-013` is now `E2E-VERIFIED`; proof: `.product/build_target/tool_artifacts/wp_runs/WP-I1-013/20260311_165018/`. It closes the specialized static family with packaged refinery, ore-processing, and water-treatment benchmarks, explicit composite-source coverage labels, grouped family summary help, map-scene projection, and recorder/bundle restore.
