...
- **Bridge discovery**: read the port file at `<app_data>/stratatlas/agent_bridge_port.txt`, verify health with `GET /agent/health`, then use navigate + action + snapshot as needed.
...
```bash
PORT=$(cat "$APPDATA/com.stratatlas.desktop/stratatlas/agent_bridge_port.txt")
