...
struct AiGatewayProviderAnalysisRequest {
    ...
    #[serde(default)]
    local_provider_config: LocalAiProviderSettings,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct AiGatewayLocalRuntimeProbeRequest {
    deployment_profile: String,
    #[serde(default)]
    local_provider_config: LocalAiProviderSettings,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct AiGatewayLocalRuntimeProbeResult {
    provider_id: String,
    runtime: String,
    provider_label: String,
    model: String,
...
const LM_STUDIO_PROVIDER_LABEL: &str = "LM Studio CLI";
const OLLAMA_PROVIDER_LABEL: &str = "Ollama CLI";
const CUSTOM_LOCAL_PROVIDER_LABEL: &str = "Custom local runtime";
...
struct ResolvedAiProvider {
    ...
    local_config: Option<ResolvedLocalAiProviderConfig>,
}

#[derive(Debug, Clone, Serialize, Deserialize, Default)]
#[serde(rename_all = "camelCase")]
struct LocalAiProviderSettings {
    #[serde(default)]
...
#[derive(Debug, Clone)]
struct ResolvedLocalAiProviderConfig {
    executable: String,
    args: Vec<String>,
...
fn normalize_ai_provider_selection(value: Option<&str>) -> String {
    match value
...
fn requested_ai_provider(provider_selection: Option<String>) -> String {
    if let Some(selection) = provider_selection {
        return normalize_ai_provider_selection(Some(selection.as_str()));
    }

    normalize_ai_provider_selection(env::var("STRATATLAS_AI_PROVIDER").ok().as_deref())
}
...
fn normalize_local_runtime_profile(value: Option<&str>) -> String {
    match value
        .and_then(trimmed_nonempty)
        .unwrap_or_else(|| DEFAULT_LOCAL_RUNTIME_PROFILE.to_string())
        .to_ascii_lowercase()
        .as_str()
    {
        "auto" | "auto_detect" => "auto_detect".to_string(),
        "lmstudio" | "lmstudio_cli" => "lmstudio_cli".to_string(),
        "ollama" | "ollama_cli" => "ollama_cli".to_string(),
        "custom" | "custom_executable" => "custom_executable".to_string(),
        _ => DEFAULT_LOCAL_RUNTIME_PROFILE.to_string(),
    }
}

fn local_runtime_label(runtime_profile: &str) -> &'static str {
    match runtime_profile {
        "lmstudio_cli" => LM_STUDIO_PROVIDER_LABEL,
        "ollama_cli" => OLLAMA_PROVIDER_LABEL,
        "custom_executable" => CUSTOM_LOCAL_PROVIDER_LABEL,
        _ => DEFAULT_LOCAL_PROVIDER_LABEL,
    }
}

fn resolve_lmstudio_executable() -> Option<String> {
    if let Some(home) = user_home_dir() {
        let candidates = [
            home.join(".lmstudio").join("bin").join("lms.exe"),
            home.join(".lmstudio").join("bin").join("lms"),
        ];
...
fn resolve_ollama_executable() -> Option<String> {
...
fn parse_listing_first_column(stdout: &str) -> Vec<String> {
...
fn select_preferred_local_model(models: &[String]) -> Option<String> {
    let pick = |needle: &str, allow_nsfw: bool| {
        models.iter().find(|model| {
            let lowered = model.to_ascii_lowercase();
            lowered.contains(needle) && (allow_nsfw || !lowered.contains("nsfw"))
        })
    };

    pick("gemma-4", false)
        .or_else(|| pick("gemma", false))
        .or_else(|| pick("gemma-4", true))
        .or_else(|| pick("gemma", true))
        .or_else(|| models.first())
        .cloned()
}

fn build_resolved_local_provider_config(
    executable: String,
    args: Vec<String>,
    model_path: Option<PathBuf>,
    ...
    workdir: Option<PathBuf>,
    detail: String,
) -> CommandResult<ResolvedLocalAiProviderConfig> {
    if (executable.contains('\\') || executable.contains('/')) && !Path::new(&executable).exists() {
        return Err(format!(
            "Configured local executable does not exist: {}",
            executable
        ));
    }
    if args.is_empty() {
        return Err("Local runtime argument list must contain at least one argument.".to_string());
    }
    ...
    if args.iter().any(|arg| arg.contains("{model_path}")) && model_path.is_none() {
        return Err(
            "Local runtime args require a model path when {model_path} is used.".to_string(),
        );
    }
    if let Some(path) = workdir.as_ref() {
        if !path.exists() {
            return Err(format!(
                "Configured local runtime working directory does not exist: {}",
                path.display()
            ));
        }
    }

    let resolved_model_label = model_label
        ...
        .unwrap_or_else(|| DEFAULT_LOCAL_PROVIDER_MODEL.to_string());

    Ok(ResolvedLocalAiProviderConfig {
        executable,
        args,
        model_path,
...
fn env_local_provider_config() -> CommandResult<ResolvedLocalAiProviderConfig> {
    let executable = env::var("STRATATLAS_LOCAL_AI_EXECUTABLE")
    .ok()
    .and_then(|value| trimmed_nonempty(&value))
    .ok_or_else(|| {
      "Set STRATATLAS_LOCAL_AI_EXECUTABLE and STRATATLAS_LOCAL_AI_ARGS_JSON to enable the governed local model provider. Supported placeholders: {prompt}, {prompt_file}, {model_path}.".to_string()
    })?;

    let raw_args = env::var("STRATATLAS_LOCAL_AI_ARGS_JSON")
    .ok()
    .and_then(|value| trimmed_nonempty(&value))
    .ok_or_else(|| {
      "Set STRATATLAS_LOCAL_AI_ARGS_JSON to a JSON string array for the governed local model provider.".to_string()
    })?;
    let args = serde_json::from_str::<Vec<String>>(&raw_args)
        .map_err(|error| format!("Invalid STRATATLAS_LOCAL_AI_ARGS_JSON: {error}"))?;

    let model_path = env::var("STRATATLAS_LOCAL_AI_MODEL_PATH")
        .ok()
        .and_then(|value| trimmed_nonempty(&value))
        .map(PathBuf::from);
    let model_label = env::var("STRATATLAS_LOCAL_AI_MODEL_LABEL")
        .ok()
        .and_then(|value| trimmed_nonempty(&value));
    let provider_label = env::var("STRATATLAS_LOCAL_AI_PROVIDER_LABEL")
        .ok()
        .and_then(|value| trimmed_nonempty(&value))
        .unwrap_or_else(|| CUSTOM_LOCAL_PROVIDER_LABEL.to_string());
    let workdir = env::var("STRATATLAS_LOCAL_AI_WORKDIR")
        .ok()
        .and_then(|value| trimmed_nonempty(&value))
        .map(PathBuf::from);

    build_resolved_local_provider_config(
        executable,
        args,
        model_path,
        model_label,
        ...
        workdir,
        format!(
            "{} is configured through environment-backed governed local runtime settings.",
            provider_label
        ),
    )
}

fn lmstudio_model_catalog(executable: &str) -> CommandResult<Vec<String>> {
    let output = Command::new(executable)
        .args(["ls"])
        .output()
        .map_err(|error| format!("Failed to query LM Studio model catalog: {error}"))?;
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
        return Err(if stderr.is_empty() {
            "LM Studio model catalog command failed.".to_string()
        } else {
            stderr
        });
    }
    Ok(parse_listing_first_column(&String::from_utf8_lossy(
        &output.stdout,
    )))
}

fn lmstudio_local_provider_config(
    settings: Option<&LocalAiProviderSettings>,
) -> CommandResult<ResolvedLocalAiProviderConfig> {
    let executable = resolve_lmstudio_executable().ok_or_else(|| {
        "LM Studio CLI was not detected. Install LM Studio CLI or switch the local runtime profile."
            .to_string()
    })?;
    let preferred_model = settings.and_then(|value| trimmed_nonempty(&value.model_identifier));
    let catalog = lmstudio_model_catalog(&executable)?;
    let model = preferred_model
    .or_else(|| select_preferred_local_model(&catalog))
    .ok_or_else(|| {
      "LM Studio was detected but no local model was discovered. Load or import a model in LM Studio first.".to_string()
    })?;
    if !catalog.is_empty()
        && !catalog
            .iter()
    ...
    {
        return Err(format!(
      "LM Studio model `{}` was not found on disk. Choose one of the installed LM Studio model keys.",
      model
    ));
    }
    ...
    build_resolved_local_provider_config(
        ...
        None,
        Some(model.clone()),
        LM_STUDIO_PROVIDER_LABEL.to_string(),
        None,
        format!(
            "{} will execute model `{}` through the governed local runtime.",
            LM_STUDIO_PROVIDER_LABEL, model
        ),
    )
}

fn ollama_model_catalog(executable: &str) -> CommandResult<Vec<String>> {
    let output = Command::new(executable)
        .args(["list"])
        .output()
        .map_err(|error| format!("Failed to query Ollama model catalog: {error}"))?;
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
        return Err(if stderr.is_empty() {
            "Ollama model catalog command failed.".to_string()
        } else {
            stderr
        });
    }
...
fn ollama_local_provider_config(
    settings: Option<&LocalAiProviderSettings>,
) -> CommandResult<ResolvedLocalAiProviderConfig> {
    let executable = resolve_ollama_executable().ok_or_else(|| {
        "Ollama CLI was not detected. Install Ollama or switch the local runtime profile."
            .to_string()
    })?;
    let preferred_model = settings.and_then(|value| trimmed_nonempty(&value.model_identifier));
    let catalog = ollama_model_catalog(&executable)?;
    let model = preferred_model
    .or_else(|| select_preferred_local_model(&catalog))
    .ok_or_else(|| {
      "Ollama was detected but no installed chat model was found. Pull a model in Ollama first."
        .to_string()
    })?;
    if !catalog.is_empty()
        && !catalog
    ...
    {
        return Err(format!(
            "Ollama model `{}` was not found. Choose a model listed by `ollama list`.",
            model
        ));
    }
    ...
    build_resolved_local_provider_config(
        executable,
        vec!["run".to_string(), model.clone(), "{prompt}".to_string()],
        None,
        Some(model.clone()),
        OLLAMA_PROVIDER_LABEL.to_string(),
        None,
        format!(
            "{} will execute model `{}` through the governed local runtime.",
            OLLAMA_PROVIDER_LABEL, model
        ),
    )
}

fn custom_local_provider_config_from_settings(
    settings: Option<&LocalAiProviderSettings>,
) -> CommandResult<ResolvedLocalAiProviderConfig> {
    let Some(settings) = settings else {
        return env_local_provider_config();
    };
    let executable = trimmed_nonempty(&settings.executable_path).ok_or_else(|| {
        "Custom local runtime requires an executable path in workspace settings.".to_string()
    })?;
    let raw_args = trimmed_nonempty(&settings.args_json).ok_or_else(|| {
        "Custom local runtime requires command args JSON in workspace settings.".to_string()
    })?;
    let args = serde_json::from_str::<Vec<String>>(&raw_args)
        .map_err(|error| format!("Invalid custom local runtime args JSON: {error}"))?;
    let model_path = trimmed_nonempty(&settings.model_path).map(PathBuf::from);
    let workdir = trimmed_nonempty(&settings.workdir).map(PathBuf::from);
    let model_label = trimmed_nonempty(&settings.model_identifier);
    ...
    build_resolved_local_provider_config(
        ...
        model_path,
        model_label,
        CUSTOM_LOCAL_PROVIDER_LABEL.to_string(),
        workdir,
        format!(
            "{} will execute the configured governed local command.",
            CUSTOM_LOCAL_PROVIDER_LABEL
        ),
    )
}

fn local_provider_config(
    settings: Option<&LocalAiProviderSettings>,
) -> CommandResult<ResolvedLocalAiProviderConfig> {
    match normalize_local_runtime_profile(settings.map(|value| value.runtime_profile.as_str())).as_str()
  {
    "lmstudio_cli" => lmstudio_local_provider_config(settings),
    "ollama_cli" => ollama_local_provider_config(settings),
    "custom_executable" => custom_local_provider_config_from_settings(settings),
    _ => env_local_provider_config()
      .or_else(|_| lmstudio_local_provider_config(settings))
      .or_else(|_| ollama_local_provider_config(settings))
      .map_err(|_| {
        "No governed local runtime was configured or detected. Use workspace settings to choose LM Studio, Ollama, or a custom executable."
          .to_string()
      }),
  }
}
...
fn local_provider_status(
    ...
    Option<ResolvedLocalAiProviderConfig>,
) {
    let runtime_profile =
        normalize_local_runtime_profile(settings.map(|value| value.runtime_profile.as_str()));
    let requested_model = settings
        .and_then(|value| trimmed_nonempty(&value.model_identifier))
        .unwrap_or_else(|| DEFAULT_LOCAL_PROVIDER_MODEL.to_string());

    match local_provider_config(settings) {
        Ok(config) => {
            let status = AiGatewayProviderStatus {
                provider_id: "local_model".to_string(),
                runtime: "tauri-live".to_string(),
                available: true,
                provider_label: config.provider_label.clone(),
                model: config.model_label.clone(),
        ...
        Err(detail) => (
            AiGatewayProviderStatus {
                ...
                runtime: "tauri-unconfigured".to_string(),
                available: false,
                provider_label: local_runtime_label(&runtime_profile).to_string(),
...
fn build_ai_gateway_probe_prompt() -> String {
    "Reply with exactly READY_LOCAL_OK and nothing else.".to_string()
}

fn resolve_local_command_args(
    config: &ResolvedLocalAiProviderConfig,
    prompt: &str,
    prompt_file: &Path,
) -> CommandResult<(Vec<String>, bool)> {
    let mut used_prompt_arg = false;
    let mut used_prompt_file = false;
    let mut resolved = Vec::with_capacity(config.args.len());

    for arg in &config.args {
        let mut next = arg.clone();
        if next.contains("{model_path}") {
            let model_path = config.model_path.as_ref().ok_or_else(|| {
                "Local provider requires a configured model path when using {model_path}."
                    .to_string()
            })?;
            next = next.replace("{model_path}", &model_path.to_string_lossy());
        }
        if next.contains("{prompt_file}") {
            used_prompt_file = true;
            next = next.replace("{prompt_file}", &prompt_file.to_string_lossy());
        }
        if next.contains("{prompt}") {
            used_prompt_arg = true;
            next = next.replace("{prompt}", prompt);
        }
        resolved.push(next);
    }

    Ok((resolved, !used_prompt_arg && !used_prompt_file))
}
...
fn strip_local_reasoning_blocks(output_text: &str) -> String {
...
fn strip_ansi_escape_sequences(output_text: &str) -> String {
    ...
    while index < bytes.len() {
        ...
        let next = output_text[index..]
            .chars()
            .next()
            .expect("character must exist while index is in bounds");
        cleaned.push(next);
        index += next.len_utf8();
    }
...
fn sanitize_local_provider_output(output_text: &str) -> String {
    let sanitized = strip_ansi_escape_sequences(&strip_local_reasoning_blocks(output_text))
        .trim()
        .to_string();
    if sanitized.is_empty() {
        strip_ansi_escape_sequences(output_text).trim().to_string()
    } else {
        sanitized
    }
}

fn execute_local_provider_command(
    config: &ResolvedLocalAiProviderConfig,
    prompt: &str,
) -> CommandResult<(String, u64)> {
    let prompt_path = env::temp_dir().join(format!("stratatlas-local-ai-{}.txt", Uuid::now_v7()));
    fs::write(&prompt_path, prompt).map_err(|error| {
        format!(
            "Failed to write local provider prompt file {}: {error}",
            prompt_path.display()
        )
    })?;

    let (args, send_prompt_via_stdin) = resolve_local_command_args(config, prompt, &prompt_path)?;
    let mut command = Command::new(&config.executable);
    command.args(args);
    if let Some(workdir) = config.workdir.as_ref() {
    ...
    let mut child = command
        .spawn()
        .map_err(|error| format!("Failed to execute local AI provider command: {error}"))?;
    if send_prompt_via_stdin {
        if let Some(mut stdin) = child.stdin.take() {
            stdin.write_all(prompt.as_bytes()).map_err(|error| {
                format!("Failed to write prompt to local AI provider stdin: {error}")
            })?;
        }
    }
    ...
    let output = child
        .wait_with_output()
        .map_err(|error| format!("Failed waiting for local AI provider command: {error}"))?;
    ...
    if !output.status.success() {
        let detail = if stderr.is_empty() {
            format!(
                "Governed local model provider failed with exit code {}",
    ...
    let output_text = sanitize_local_provider_output(&stdout);
    if output_text.is_empty() {
        return Err("Governed local model provider returned an empty response".to_string());
    }

    Ok((output_text, duration_ms))
}

fn run_local_provider_analysis(
    ...
    let (output_text, _) = execute_local_provider_command(config, &prompt)?;
...
fn probe_local_provider_runtime(
    request: &AiGatewayLocalRuntimeProbeRequest,
) -> AiGatewayLocalRuntimeProbeResult {
    let probed_at = Utc::now().to_rfc3339();
    if !matches!(request.deployment_profile.as_str(), "connected" | "restricted") {
        return AiGatewayLocalRuntimeProbeResult {
            provider_id: "local_model".to_string(),
            runtime: "tauri-unconfigured".to_string(),
            provider_label: local_runtime_label(&normalize_local_runtime_profile(Some(
                request.local_provider_config.runtime_profile.as_str(),
            )))
            .to_string(),
            model: trimmed_nonempty(&request.local_provider_config.model_identifier)
                .unwrap_or_else(|| DEFAULT_LOCAL_PROVIDER_MODEL.to_string()),
            outcome: "unavailable".to_string(),
            detail:
                "The current deployment profile blocks governed AI execution, so the local runtime probe is unavailable."
                    .to_string(),
            output_text: None,
            duration_ms: None,
            probed_at,
    ...
    let (status, local_config) = local_provider_status(Some(&request.local_provider_config));
    ...
    match execute_local_provider_command(&config, &build_ai_gateway_probe_prompt()) {
        Ok((output_text, duration_ms)) => AiGatewayLocalRuntimeProbeResult {
            provider_id: "local_model".to_string(),
            runtime: "tauri-live".to_string(),
            provider_label: status.provider_label,
            model: status.model,
            ...
            detail: format!(
                "{} executed a governed local runtime probe successfully.",
        ...
        Err(detail) => AiGatewayLocalRuntimeProbeResult {
            provider_id: "local_model".to_string(),
            runtime: "tauri-live".to_string(),
            provider_label: status.provider_label,
            model: status.model,
...
fn run_codex_cli_provider_analysis(
    ...
    let output_text = fs::read_to_string(&output_path).map_err(|error| {
        format!(
            "Failed to read Codex CLI provider output {}: {error}",
            output_path.display()
        )
    })?;
    let _ = fs::remove_file(&output_path);
...
fn bundles_dir(app: &AppHandle) -> CommandResult<PathBuf> {
    ...
    fs::create_dir_all(&path).map_err(|error| {
        format!(
            "Failed to create bundles directory {}: {error}",
            path.display()
        )
    })?;
    Ok(path)
...
fn exports_dir(app: &AppHandle) -> CommandResult<PathBuf> {
    ...
    fs::create_dir_all(&path).map_err(|error| {
        format!(
            "Failed to create exports directory {}: {error}",
            path.display()
        )
    })?;
    Ok(path)
...
fn initialize_control_plane_schema(client: &mut Client) -> CommandResult<()> {
    ...
    client.batch_execute(&sql).map_err(|error| {
        format!("Failed initializing PostgreSQL/PostGIS control plane schema: {error}")
    })?;
...
fn upsert_governed_deployment_profiles(client: &mut Client) -> CommandResult<()> {
    ...
    for profile in governed_deployment_profiles() {
        ...
        client
            ...
            .map_err(|error| {
                format!(
                    "Failed upserting deployment profile {}: {error}",
                    profile.id
                )
            })?;
    }
...
fn strategic_action_score(
    ...
    let category_weight = match category {
        "logistics" => {
            (context_record_count as f64 * 1.8) + if deviation_present { 9.5 } else { 2.0 }
        }
        "policy" => (information_count * 3.4) + (objective_weight * 5.0),
        _ => (threshold_ref_count as f64 * 2.8) + (chance_count * 1.4),
    };
    58.0 + (objective_weight * 20.0) + (decision_count * 3.5) + (information_count * 2.5)
        - (chance_count * 1.3)
        + category_weight
        + method_modifier
...
fn run_governed_strategic_solver(request: &StrategicSolverRequest) -> CommandResult<Value> {
    ...
    for actor in &actors {
        ...
        if !matches!(
            actor_type,
            "state" | "bloc" | "institution" | "industry_coalition"
        ) {
    ...
    let iteration_limit =
        value_number(solver_config_value.get("iteration_limit"), 25.0).round() as i64;
    let parameter_range_notes =
        value_string_array_from_entry(solver_config_value.get("parameter_range_notes"));
    ...
    model.insert("version".to_string(), json!(current_version + 1));
...
fn persist_control_plane_from_state(
    client: &mut Client,
    state: &RecorderState,
) -> CommandResult<()> {
    let quoted = quoted_schema(&control_plane_schema()?);
    ...
    for record in extract_value_array(&state.context, "records") {
        ...
        let observed_at = parse_rfc3339(&value_string_field(
            &record,
            "observed_at",
            "context record",
        )?)?;
        let cached_at =
            parse_rfc3339(&value_string_field(&record, "cached_at", "context record")?)?;
    ...
    for aoi_id in aoi_ids {
        ...
        client
            .execute(
                &upsert_aoi_query,
                &[&aoi_id, &label, &footprint, &Utc::now()],
            )
...
fn upsert_bundle_registry_entry(
    client: &mut Client,
    manifest: &BundleManifest,
) -> CommandResult<()> {
    let quoted = quoted_schema(&control_plane_schema()?);
    ...
    client
        ...
        .map_err(|error| {
            format!(
                "Failed upserting bundle registry entry {}: {error}",
                manifest.bundle_id
            )
        })?;
    Ok(())
...
fn load_control_plane_state_from_db(
    client: &mut Client,
    app: &AppHandle,
) -> CommandResult<ControlPlaneState> {
    let quoted = quoted_schema(&control_plane_schema()?);
    ...
    let context_domain_registry = client
        .query(
            &format!(
                "SELECT domain_json::text FROM {quoted}.context_domains ORDER BY domain_id ASC"
            ),
            &[],
        )
...
fn query_context_records_from_db(
    ...
    let records = client
        .query(
            &query,
            &[&request.domain_ids, &request.target_id, &start, &end],
        )
...
fn copy_directory_recursive(source: &Path, destination: &Path) -> CommandResult<()> {
    ...
    for entry in fs::read_dir(source).map_err(|error| {
        format!(
            "Failed reading runtime smoke directory {}: {error}",
            source.display()
        )
    })? {
        let entry = entry.map_err(|error| format!("Failed reading directory entry: {error}"))?;
...
fn read_audit_head(app: &AppHandle) -> CommandResult<AuditHead> {
    if let Some(mut client) = control_plane_connect()? {
        let quoted = quoted_schema(&control_plane_schema()?);
        let query = format!("SELECT event_hash FROM {quoted}.audit_head WHERE head_key = $1");
        let row = client
...
fn save_recorder_state_internal(
    app: &AppHandle,
    state: &RecorderState,
) -> CommandResult<RecorderState> {
    let path = recorder_state_path(app)?;
...
fn write_bundle_asset<T: Serialize>(
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|error| {
            format!(
                "Failed creating asset directory {}: {error}",
                parent.display()
            )
        })?;
    }
...
fn read_bundle_asset_bytes(bundle_root: &Path, asset: &BundleAsset) -> CommandResult<Vec<u8>> {
    ...
    fs::read(&asset_path).map_err(|error| {
        format!(
            "Failed reading bundle asset {}: {error}",
            asset_path.display()
        )
    })
}
...
#[tauri::command]
fn probe_local_ai_runtime(
    request: AiGatewayLocalRuntimeProbeRequest,
) -> CommandResult<AiGatewayLocalRuntimeProbeResult> {
    Ok(probe_local_provider_runtime(&request))
}
    ...
    match provider.kind {
        ...
        LiveAiProviderKind::LocalModel => {
            let local_config = provider.local_config.as_ref().ok_or_else(|| {
                "Local provider status resolved without a runnable local configuration.".to_string()
            })?;
            run_local_provider_analysis(&request, &provider.status, local_config)
        }
    }
}
            ...
            domain_id
                .map(|value| {
                    request
                        .domain_ids
                        .iter()
                        .any(|domain_id| domain_id == value)
                })
                .unwrap_or(false)
                && target_id
                    .map(|value| value == request.target_id)
                    .unwrap_or(false)
                && observed_at
                    .map(|value| {
...
async fn fetch_commercial_air_traffic_internal(
    ...
    let source_state = if mean_age_seconds <= 90 {
        "live"
    } else {
        "delayed"
    };
...
async fn fetch_satellite_elements_internal(
    ...
    for norad_cat_id in unique_ids {
        let source_url =
            format!("https://celestrak.org/NORAD/elements/gp.php?CATNR={norad_cat_id}&FORMAT=JSON");
        let response = client
            .get(&source_url)
        ...
        let response_json = response.json::<Value>().await.map_err(|error| {
            format!("Failed to parse satellite response for CATNR {norad_cat_id}: {error}")
        })?;
        let Some(rows) = response_json.as_array() else {
            warnings.push(format!(
                "CATNR {norad_cat_id} returned a non-array payload."
            ));
            continue;
        };
        ...
        if rows.is_empty() {
            warnings.push(format!(
                "CATNR {norad_cat_id} returned no orbital elements."
            ));
            continue;
        }
...
fn write_runtime_smoke_evidence(
    ...
    if let Some(mut client) = control_plane_connect()? {
        ...
        let audit_head = client
            ...
            .map_err(|error| {
                format!("Failed reading PostgreSQL audit head for proof export: {error}")
            })?
            .and_then(|row| row.get::<_, Option<String>>(0));
        ...
        let audit_events_query =
            format!("SELECT event_json::text FROM {quoted}.audit_events ORDER BY ts ASC");
        let audit_lines = client
            .query(&audit_events_query, &[])
            .map_err(|error| {
                format!("Failed reading PostgreSQL audit events for proof export: {error}")
            })?
            .iter()
        ...
        if let Some(row) = client
            ...
            .map_err(|error| {
                format!("Failed reading PostgreSQL recorder state for proof export: {error}")
            })?
        {
            fs::write(
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct AgentBridgeActionRequest {
    action: String,
    #[serde(default)]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct AgentBridgeActionCompletion {
    action: String,
    success: bool,
    #[serde(default)]
...
struct AgentBridgeInner {
    ...
    action_tx: Option<std::sync::mpsc::Sender<AgentBridgeActionCompletion>>,
}
...
#[tauri::command]
fn agent_action_complete(completion: AgentBridgeActionCompletion) {
    let mut state = agent_bridge_state().lock().unwrap();
    if let Some(tx) = state.action_tx.take() {
    ...
    let (status, resp) = match (method, path) {
        ...
        ("POST", "/agent/snapshot") => {
            ...
            match rx.recv_timeout(Duration::from_secs(30)) {
                ...
                Err(_) => {
                    ...
                    (
                        "504 Gateway Timeout",
                        json!({ "error": "snapshot timed out" }).to_string(),
                    )
                }
            }
        }
        ("POST", "/agent/action") => {
            ...
            if action.is_empty() {
                (
                    "400 Bad Request",
                    json!({ "error": "missing action" }).to_string(),
                )
            } else {
                ...
                let (tx, rx) = std::sync::mpsc::channel::<AgentBridgeActionCompletion>();
                {
                    agent_bridge_state().lock().unwrap().action_tx = Some(tx);
                }
                if let Some(app) = AGENT_APP_HANDLE.get() {
                    let _ = app.emit(
                        "agent-action-request",
                        AgentBridgeActionRequest {
                            action: action.clone(),
                            payload,
                        },
                    );
                    match rx.recv_timeout(Duration::from_millis(timeout_ms)) {
                        Ok(completion) => (
                            "200 OK",
                            serde_json::to_string(&completion).unwrap_or_else(|_| {
                                json!({ "error": "action response serialization failed" })
                                    .to_string()
                            }),
                        ),
                        Err(_) => {
                            agent_bridge_state().lock().unwrap().action_tx = None;
                            (
                                "504 Gateway Timeout",
                                json!({ "error": "action timed out", "action": action })
                                    .to_string(),
                            )
                        }
                    }
                    ...
                    (
                        ...
                        json!({ "error": "agent bridge app handle unavailable", "action": action })
                            .to_string(),
                    )
                }
            }
...
mod tests {
    use super::{
        agent_bridge_state, agent_action_complete, canonical_json_bytes, control_plane_connect,
        control_plane_schema, ensure_valid_relative_path, persist_control_plane_from_state,
        query_context_records_from_db, quoted_schema, sanitize_local_provider_output, sha256_hex,
        strip_ansi_escape_sequences, strip_local_reasoning_blocks, upsert_bundle_registry_entry,
        AgentBridgeActionCompletion, BundleAsset, BundleManifest, ContextTimeRange, ProvenanceRef,
        QueryContextRecordsRequest, RecorderState,
    };
    use serde_json::json;
    ...
    #[test]
    fn agent_action_completion_forwards_structured_payload() {
        let (tx, rx) = std::sync::mpsc::channel::<AgentBridgeActionCompletion>();
        {
            let mut state = agent_bridge_state().lock().unwrap();
        ...
        agent_action_complete(AgentBridgeActionCompletion {
            action: "probe-local-runtime".to_string(),
            success: true,
            message: Some("Local runtime probe completed.".to_string()),
            result: Some(json!({
                "outcome": "success",
                "runtime": "tauri-live"
            })),
        ...
        let completion = rx
            ...
            .expect("agent action completion should be forwarded");
        assert_eq!(completion.action, "probe-local-runtime");
    ...
    #[test]
    fn strip_local_reasoning_blocks_removes_think_sections() {
        let output = "<think>\ninternal reasoning\n</think>\nREADY_LOCAL_OK";

        assert_eq!(strip_local_reasoning_blocks(output), "READY_LOCAL_OK");
    }

    #[test]
    fn strip_local_reasoning_blocks_preserves_plain_output() {
        let output = "Interpretive output only.";

        assert_eq!(
            strip_local_reasoning_blocks(output),
            "Interpretive output only."
        );
    }

    #[test]
    fn strip_ansi_escape_sequences_removes_terminal_control_codes() {
        let output = "\u{1b}[K\u{1b}[?25hREADY_LOCAL_OK";

        assert_eq!(strip_ansi_escape_sequences(output), "READY_LOCAL_OK");
    }

    #[test]
    fn sanitize_local_provider_output_removes_reasoning_and_terminal_codes() {
        let output = "<think>\ninternal reasoning\n</think>\n\u{1b}[KREADY_LOCAL_OK";

        assert_eq!(sanitize_local_provider_output(output), "READY_LOCAL_OK");
    }
    ...
    fn live_control_plane_query_roundtrip() {
        ...
        let supersedes_query = format!(
            "SELECT supersedes_bundle_id FROM {quoted}.bundle_registry WHERE bundle_id = $1"
        );
